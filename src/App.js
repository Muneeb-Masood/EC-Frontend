...
const App = () => {
        ...
        const fetchKycRequests = async () => {
            try {
                const response = await apiRequest(
                    "GET",
                    "http://localhost:5000/api/getKyc"
                );
                ...
                if (Array.isArray(response.data.kycRequests)) {
                    ...
                    setDisplayMessage({
                        type: "error",
                        message: "KYC data is not in the expected format.",
                    });
                }
            } catch (error) {
                setDisplayMessage({
    ...
    const handleKYCSubmit = async (kycData) => {
        ...
        const result = await apiRequest(
            "POST",
            "http://localhost:5000/api/kyc/uploadDocs",
            formData,
    ...
    const handleApproveKYC = async (kycID) => {
        ...
        const response = await apiRequest(
            "POST",
            "http://localhost:5000/api/kyc/approve",
            { kycID }
        );

        if (response.success) {
            const updatedRequests = kycRequests.filter(
                (request) => request.kycID !== kycID
            );
    ...
    const submitRejection = async () => {
        ...
        const response = await apiRequest(
            "POST",
            "http://localhost:5000/api/kyc/reject",
            {
                kycID: selectedKycID,
        ...
        if (response.success) {
            const updatedRequests = kycRequests.filter(
                (request) => request.kycID !== selectedKycID
            );
            setKycRequests(updatedRequests);
            ...
            <div className="app">
                <Routes>
                    ...
                    <Route
                        path="/kyc"
                        element={
                            kycVerificaionStatus === "verified" ? (
                                <Navigate to="/dashboard" />
                            ) : isLoggedIn ? (
                                <KYCForm
                                    onKYCSubmit={handleKYCSubmit}
                                    kycVerificaionStatus={kycVerificaionStatus}
                                />
                            ) : (
                                <Navigate to="/login" />
                            )
                    ...
                    <div className="modalOverlay">
                        <div className="modalContent">
                            ...
                            <div className="modalButtons">
                                <button
                                    className="modalButton rejectConfirm"
                                    onClick={submitRejection}
                                >
                                    Submit
                                </button>
                                <button
                                    className="modalButton cancelModal"
